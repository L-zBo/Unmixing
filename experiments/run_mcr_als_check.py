"""Quick MCR-ALS check on synthetic ground-truth data (alongside NNLS / FCLS / PRISM)."""
...
DEFAULT_OUTPUT_ROOT = ROOT / "outputs/experiments/mcr_als_check"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--mcr-max-iter", type=int, default=100)
...
def evaluate(method_name: str, truth_flat: np.ndarray, abundances: np.ndarray,
             reconstructed: np.ndarray, spectra_norm: np.ndarray,
             height: int, width: int, elapsed: float, extra: dict | None = None) -> dict:
...
def format_table(rows: list[dict]) -> str:
    cols = ["method", "mae", "rmse", "pearson_r", "recon_rmse", "spatial_tv",
            "mean_active_endmembers", "elapsed_s"]
    lines = ["\t".join(cols)]
    for r in rows:
        lines.append("\t".join(
...
def main() -> None:
    ...
    ols_result = unmix_spectra(spectra_norm, library=library, method="ols")
    rows.append(evaluate("ols", truth_flat, ols_result.abundances, ols_result.reconstructed,
    ...
    t0 = time.perf_counter()
    fcls_result = unmix_spectra(spectra_norm, library=library, method="fcls")
    rows.append(evaluate("fcls", truth_flat, fcls_result.abundances, fcls_result.reconstructed,
                         spectra_norm, height, width, time.perf_counter() - t0))

    t0 = time.perf_counter()
    nmf_raw = blind_nmf_unmix_spectra(spectra_norm, n_components=library.n_endmembers)
    nmf_result, _ = align_blind_nmf_to_reference(nmf_raw, reference_library=library)
    rows.append(evaluate("nmf", truth_flat, nmf_result.abundances, nmf_result.reconstructed,
                         spectra_norm, height, width, time.perf_counter() - t0))

    t0 = time.perf_counter()
    mcr_hard_sum1 = mcr_als_unmix_spectra(spectra_norm, library=library,
                                          fix_endmembers=True, sum_to_one=True,
                                          max_iter=args.mcr_max_iter)
    rows.append(evaluate("mcr_als_hard_sum1", truth_flat, mcr_hard_sum1.abundances,
                         mcr_hard_sum1.reconstructed, spectra_norm, height, width,
                         time.perf_counter() - t0,
                         extra={"n_iter": mcr_hard_sum1.n_iter}))

    t0 = time.perf_counter()
    mcr_hard_nnls = mcr_als_unmix_spectra(spectra_norm, library=library,
                                          fix_endmembers=True, sum_to_one=False,
                                          max_iter=args.mcr_max_iter)
    rows.append(evaluate("mcr_als_hard_nnls", truth_flat, mcr_hard_nnls.abundances,
                         mcr_hard_nnls.reconstructed, spectra_norm, height, width,
                         time.perf_counter() - t0,
                         extra={"n_iter": mcr_hard_nnls.n_iter}))

    t0 = time.perf_counter()
    mcr_semi = mcr_als_unmix_spectra(spectra_norm, library=library,
                                     fix_endmembers=False, sum_to_one=True,
                                     max_iter=args.mcr_max_iter)
    drift = mcr_semi.config["st_opt_drift_sam_rad"]
    rows.append(evaluate("mcr_als_semi_blind", truth_flat, mcr_semi.abundances,
                         mcr_semi.reconstructed, spectra_norm, height, width,
                         time.perf_counter() - t0,
                         extra={"n_iter": mcr_semi.n_iter,
                                "endmember_drift_max_rad": max(drift) if drift else 0.0}))
    ...
    df.to_csv(args.output_root / "mcr_als_check_summary.csv", index=False, encoding="utf-8-sig")
    print(f"\n=== MCR-ALS check on {args.synthetic_root.name} ({height}x{width}) ===")
    ...
    summary_payload = {
        ...
        "mcr_max_iter": int(args.mcr_max_iter),
        "lambda_l2": float(args.lambda_l2),
        "lambda_tv": float(args.lambda_tv),
        "tv_iters": int(args.tv_iters),
        "rows": rows,
    }
    (args.output_root / "mcr_als_check_summary.json").write_text(
